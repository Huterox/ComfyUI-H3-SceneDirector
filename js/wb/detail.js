...
export function createDetail({ store, backend, getSelected }) {
    ...
    // 一次性文件选择（首帧上传用；图钉沿用下面常驻的 fileInput）
    ...
    function renderSide(i) {
        ...
        // i2v 首帧源图（仅图生视频模式显示；上传写 seg.firstFrame，
        // 适配层落到 Director 段的 genImage）
        if (store.get().mode === "i2v") {
            const ffWrap = el("div");
            ffWrap.appendChild(el("label", "h3wb-label", "首帧源图（i2v）"));
            const box = el("div", "h3wb-ffbox");
            const paint = () => {
                box.innerHTML = "";
                const ff = seg?.firstFrame;
                if (ff && ff.image) {
                    const img = document.createElement("img");
                    img.src = backend.inputURL(ff);
                    box.appendChild(img);
                    const x = el("u", null, "×");
                    x.title = "清除首帧（退回文生视频）";
                    ...
                    box.classList.add("h3wb-ffempty");
            ...
            box.addEventListener("click", async () => {
                ...
                try {
                    const info = await backend.uploadImage(file);
                    seg.firstFrame = { image: info.name, subfolder: info.subfolder || "" };
                    store.commit({ structural: true });
                } catch (e2) {
                    console.error("[h3-workbench] 首帧上传失败", e2);
                }
            });
            ffWrap.appendChild(box);
            side.appendChild(ffWrap);
        }
    ...
    // 例外：外部改写（增强器应用结果/工作流加载）时同步——用户没在编辑且
    // 内容确实不同才覆盖，绝不抢输入焦点。
    function setSegment(i) {
        if (i === cur) {
            const seg = store.get().segments[i];
            if (seg && document.activeElement !== ta && ta.value !== seg.prompt) {
                ta.value = seg.prompt;
            }
            renderSide(i);
            return;
