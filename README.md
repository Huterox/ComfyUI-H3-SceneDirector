...
以及**可组合节点架构**——不做上帝节点，model / sampler /
...
- **项目栏**：整台状态（全局设定 + 资产库 + 六模式分镜舱）一键存为项目，
  下拉即换项目，不用再复制工作流；未保存的改动在应用 AI 产物时自动落盘起名
- **资产库**：角色 / 场景 / 物品参考素材统一管理，两种语义——
  `📌 常驻`（每段必注入，身份锚）与 `○ 按需`（段内 `@` 引用才进模型）；
  提示词输入 `@` 即时补全资产名
- 载荷 v5：旧工作流打开自动迁移，分镜数据零丢失

**智能体（项目 agent，pi-ai 驱动）**

- **服务配置**弹窗：LLM 端点双格式（OpenAI 兼容 / Anthropic）+ 上下文窗口
  设置（超限自动压缩）；生图服务两家（Seedream / nanobanana），配置随项目保存
- **对话改写**（每段魔法棒）：不是一次性模板扩写，而是和项目 agent 对话——
  先改不满意就接着聊，改到满意再应用；agent 记得整个项目的上下文
- **AI 自动创作**：一句话想法 → ReAct agent 自主调用工具（看工作台 / 切模式 /
  写全局 / 加资产 / 生图 / 排分镜 / 反问你）产出整台草稿；步骤流可视、
  产物随出随看，确认后一键"应用到工作台"（资产库 + 分镜全量回填）
- 会话按项目绑定（SQLite 持久化），对话改写与自动创作共享同一 session——
  agent 全程记得这个项目里发生过什么；换项目即换会话，可一键重置
- agent 内置双技能：H3 提示词写作规范（文生/参考两篇）+ SceneDirector
  工作台使用手册，写出的提示词直接符合模型口味与工作台结构
...
AI 自动创作抽屉（步骤流 + 产物预览 + 多轮对话 + 一键应用）：

![AI 自动创作](screenshots/workbench_autopilot.png)
...
点工作台右上「服务配置」弹窗（配置随项目保存，密钥只进本地配置文件，
不会被打包进工作流 JSON）：

| 区块 | 字段 | 说明 |
|---|---|---|
| LLM | API 格式 / URL / Key / 模型 | OpenAI 兼容或 Anthropic Messages 两种协议 |
| LLM | 上下文窗口 | token 上限，占用超 80% 自动压缩历史，长项目不爆 |
| 生图 | 服务商 / URL / Key / 模型 | Seedream（images 协议）或 nanobanana（chat 协议），自动创作时 agent 自行调用出参考图 |

不配 LLM 也能正常使用全部手动功能；智能体相关入口会提示"LLM 未配置"。

agent 会话存于 `user/SceneDirector/agent_sessions.db`，按项目隔离；
「重置会话」清空当前项目的对话记忆（分镜数据不受影响）。
